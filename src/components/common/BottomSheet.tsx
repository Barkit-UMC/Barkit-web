  ...
  return (
    <div className="absolute inset-0 z-[9999] flex items-end justify-center">
      ...
      <div className="relative w-full max-w-md bg-white rounded-t-[40px] pt-8 pb-8 px-6 shadow-2xl">
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-4 py-4 transition-all active:opacity-60"
              >
                {/* 아이콘*/}
                <img
                  src={option.icon}
        ...
        <button
          onClick={onClose}
          className="w-full bg-[#f2fcfe] py-4 rounded-full text-[#00C0E8] font-medium text-xl"
        >
          취소
        </button>
      </div>
